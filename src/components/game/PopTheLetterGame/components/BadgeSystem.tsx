...
const BadgeSystem = ({ badges, recentBadge }: BadgeSystemProps) => {
  ...
  return (
    <Card className="p-4 bg-gradient-to-r from-pink-50 to-purple-50">
      <h3 className="font-fredoka font-bold text-lg mb-3 text-center">🏅 Letter Badges</h3>
      
      {recentBadge && (
        <div className="mb-4 p-3 bg-pink-100 rounded-lg text-center animate-pulse">
          <div className="text-2xl mb-1">🎉</div>
          <div className="font-comic font-bold text-pink-700">New Badge Earned!</div>
          <div className="font-comic text-sm text-pink-600">{recentBadge}</div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {badges.map((badge) => (
          ...
          >
            <div className={`mb-2 ${badge.earned ? 'text-pink-600' : 'text-gray-400'}`}>
              {getIcon(badge.icon)}
            </div>
