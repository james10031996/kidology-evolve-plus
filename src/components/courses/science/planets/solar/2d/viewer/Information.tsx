...
export default function Information({ currentPlanet, showInfo, setShowInfo }: InformationProps) {
  return (
    <>
      <button
        aria-label="open Information section"
        onClick={() => setShowInfo(true)}
        className="fixed top-28 left-0 z-40 bg-white hover:bg-gray-100 px-1 py-4 rounded-r-md"
      >
        <Menu className="text-black transform rotate-90" fontSize="medium" />
      </button>
      ...
      {showInfo && (
        <div className="fixed top-0 left-0 z-50 h-dvh w-[320px] bg-white text-black transition-transform duration-300 overflow-y-auto">
          <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
            ...
            <button
              aria-label="close Information section"
