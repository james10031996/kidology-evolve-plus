...
const output = document.getElementById("demo") as HTMLElement;
...
slider.oninput = () => {
  output.innerHTML = slider.value;
  orbitFactor = parseFloat(slider.value);
  rotateFactor = parseFloat(slider.value);
};
...
export default function ThreeDSolarSystem() {
  useEffect(() => {
    System(); // Run your Three.js logic once after mount
  }, []);

  return (
    <div id="three-container" className="w-full h-screen bg-black" />
  );
}
